...
/*   
	...
	FILE NAME: AutoWho_ViewHistoricalSessions.StoredProcedure.sql

	PROCEDURE NAME: AutoWho_ViewHistoricalSessions
	...
	BEGIN
		...
		BEGIN
			...
			SELECT convert(xml,N'<?optqry --' + NCHAR(10) +
						...
						N' -- ' + NCHAR(10) + NCHAR(13) + 
						N'SELECT * FROM @@CHIRHO_SCHEMA@@.AutoWho_CaptureTimes ct WITH (NOLOCK) ORDER BY ct.SPIDCaptureTime DESC;
SELECT * FROM AutoWho.Log l WITH (NOLOCK) ORDER BY l.LogDT DESC;
SELECT * FROM @@CHIRHO_SCHEMA@@.AutoWho_SessionsAndRequests sar WITH (NOLOCK) WHERE sar.SPIDCaptureTime = ''' + 
...
SELECT * FROM @@CHIRHO_SCHEMA@@.AutoWho_TasksAndWaits taw WITH (NOLOCK) WHERE taw.SPIDCaptureTime = ''' + 
	REPLACE(CONVERT(NVARCHAR(20), @hct, 102),'.','-')+' '+CONVERT(NVARCHAR(20), @hct, 108)+'.'+RIGHT(CONVERT(NVARCHAR(20),N'000')+CONVERT(NVARCHAR(20),DATEPART(MILLISECOND, @hct)),3)+N''';
SELECT * FROM @@CHIRHO_SCHEMA@@.AutoWho_TransactionDetails td WITH (NOLOCK) WHERE td.SPIDCaptureTime = ''' + 
	REPLACE(CONVERT(NVARCHAR(20), @hct, 102),'.','-')+' '+CONVERT(NVARCHAR(20), @hct, 108)+'.'+RIGHT(CONVERT(NVARCHAR(20),N'000')+CONVERT(NVARCHAR(20),DATEPART(MILLISECOND, @hct)),3)+N''';
SELECT * FROM @@CHIRHO_SCHEMA@@.AutoWho_LockDetails ld WITH (NOLOCK) WHERE ld.SPIDCaptureTime = ''' + 
	REPLACE(CONVERT(NVARCHAR(20), @hct, 102),'.','-')+' '+CONVERT(NVARCHAR(20), @hct, 108)+'.'+RIGHT(CONVERT(NVARCHAR(20),N'000')+CONVERT(NVARCHAR(20),DATEPART(MILLISECOND, @hct)),3)+N''';
